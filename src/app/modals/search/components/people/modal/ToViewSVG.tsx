...
import { MoreButton } from "@/app/(__pages__)/popular-people/components/MoreButton";
...
export  default function ToViewSVG({
  ...
  const [displayHeight, setDisplayHeight] = useState(window.innerHeight/2);
  const [preferedHeight, setPreferedHeight] = useState(window.innerHeight);
  ...
  const [cardParentWidth, setCardParentWidth] = useState(
    cardParentRef?.current?.clientWidth || 0)
  ;


  // console.log(moviesOrTVshows)
  useEffect(() => {
    const itemsContainerParent = itemsContainerParentRef.current;
    if (!itemsContainerParent) return
    
    const fn = () => {
      if (!isActiveToViewSVG) {
        itemsContainerParent.style.overflow = 'auto'
        // itemsContainerParent.scrollTo(0, coordsAndInfo.scrollY)
        return 
      }
      
      itemsContainerParent.scrollTo(0, 0)
      const isTop = itemsContainerParent.scrollTop === 0;
      if (!isTop) return

      itemsContainerParent.style.overflow = 'hidden'
    }
    fn()

    itemsContainerParent.addEventListener('scroll', fn)
    return () => {
      itemsContainerParent.removeEventListener('scroll', fn)
    }
  }, [isActiveToViewSVG, itemsContainerParentRef]);
...
// resize
  useEffect(() => {
    ...
    // console.log(itemsContainerParent.clientHeight, itemsContainerParent.scrollHeight, itemsContainerParent.scrollTop,  innerHeight)

    const fn = () => {
      // console.log('zaa')
      const {top, height, x, y, width} = itemsContainerParent.getBoundingClientRect();
      setPreferedHeight(itemsContainerParent.scrollTop)
      setDisplayHeight(innerHeight-top)
      setParentBounds({top, height, x, y, width})
  ...
  useEffect(() => {
    const cardParent = cardParentRef.current;
    if (!cardParent) return;
    
    // console.log(cardParent.getBoundingClientRect(), cardParent.clientWidth)
    
    
    function fn () {
      if (!cardParent) return;
      setCardParentWidth(0);
      setCardParentWidth(cardParent.clientWidth || 0);
    }
    fn()

    cardParent.addEventListener('resize', fn)
    return () => {
      cardParent.removeEventListener('resize', fn)
    }

  }, [ cardParentRef, innerWidth]);
...
// resize
  ...
  //   ? coordsAndInfo.width * 4 //1.7 
  //   : coordsAndInfo.width * 2.1 //1.5
  // ;
  const biggerWidth = innerWidth 
  ...
  // ;
  // const bigerHeight = preferedHeight > 500
  //   ? (coordsAndInfo.height * 3 < preferedHeight)? coordsAndInfo.height * 3: preferedHeight // 2
  //   : coordsAndInfo.height * 1.5 // 1.1
  // ;
  const bigerHeight = displayHeight*0.83 // 0.83
  ...
  // console.log('scrollY', coordsAndInfo.scrollY)
  // console.log('scrollHeight', preferedHeight)
  // console.log('innerHeight', innerHeight)
  // console.log('height', coordsAndInfo.height)
  // console.log('displayHeight', displayHeight)
  // console.log('biggerHeight', bigerHeight)
  // console.log('zz',parentBounds)
  if (!isActiveToViewSVG) return null

  // let abc =moveToCenter && isActiveToViewSVG
  //         ? ((parentBounds.width) - (cardParentWidth
  //           ? cardParentWidth : coordsAndInfo.width))/2.5 + 'px' 
  //         : coordsAndInfo.x + 'px';

  // let abc =moveToCenter && isActiveToViewSVG
  //         ? (parentBounds.width/2 - cardParentWidth/2) + 'px' :'0px';

  // console.log(parentBounds.width, cardParentWidth, coordsAndInfo)
  // console.log(coordsAndInfo.scrollY, coordsAndInfo.scrollHeight)

  let negativeOrPositiveMathSign = 
  ...
  return (
    <div 
      ...
      className={`to-view-svg bg-red-500p  rounded-2xl absolute top-0 transition-[left_top_width_height_scale] ${moveToCenter? 'duration-[200ms]' : 'duration-[500ms]'}  z-[${isActiveToViewSVG? '10' : '-1'}] 
      ${isActiveToViewSVG? 'scale-100' : 'scale-0'} origin-[100%_50%] bg-red-500p border-green-500p mx-auto justify-center place-content-center w-full`}
      style={{
        // left: moveToCenter && isActiveToViewSVG
        //   ? ((innerWidth / 2) - ((biggerWidth) / 2)) + 'px' 
        //   : coordsAndInfo.x + 'px',
        left: moveToCenter && isActiveToViewSVG
        ? `${0}px` : `${negativeOrPositiveMathSign+ coordsAndInfo.x*Math.random()*40}px`,
   
        top: moveToCenter && isActiveToViewSVG
          ? `${0}px` : `${negativeOrPositiveMathSign+ coordsAndInfo.y*Math.random()*40}px`,
    ...
    >
      ...
      <div className="w-max absolute top-0 left-1/2 -translate-x-1/2 mx-auto">
        <svg 
        viewBox="0 0 173 246" fill="none" xmlns="http://www.w3.org/2000/svg"
          ref={svgRef}
          // onClick={handleCardClick} 
          className="cursor-pointerp bg-emerald-500p scale-[1.19]p origin-top w-[85vmin] sm:w-[52vmin]"
          >
          ...
          >
            ...
            <g>
              ...
              <foreignObject  x="3" y="78" width="167" height="165">
                <div className="px-1p h-[45%] grid text-[8px] leading-[12px] py-2 pt-0">
        ...
        <div className="absolute bottom-[3px] sm:bottom-[7px] left-0 right-0 overflow-hidden"
          onClick={e => e.stopPropagation()}
        >
          {
          cardData?.known_for &&
          <div className="grid grid-cols-3 gap-1 overflow-hidden rounded-b-[8%] mx-[10px] sm:mx-[15px] h-max"
          >
            {
