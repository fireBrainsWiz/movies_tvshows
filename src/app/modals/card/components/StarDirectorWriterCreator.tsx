...
import { getGenderByNumber, starDirectorWriterCreator } from "../lib/utils"
import { MediaTypeInfoType } from "@/app/lib/MediaTypeInfoTypes"
import { CombinedCredits, TrendingPeopleData, getBestCombinedCredit } from "@/app/(__pages__)/(top_links)/popular/trending/TrendingPeople"
...
import { PreparePersonData } from "@/app/(__pages__)/(top_links)/popular/trending/PreparePersonData"
import { StarDirectorWriterResultsType } from "./StarDirectorWriterCreatorBar"
...
export default memo( function StarDirectorWriterCreator(
  {
    ...
    details,
    selected,
    setStarDirectorWriterResults
  }: {
    credits: MediaTypeInfoType['credits'],
    details: MediaTypeInfoType['details']
    selected: ButtonOption
    setStarDirectorWriterResults: (value: StarDirectorWriterResultsType | null) => void
  }) {

    const creator = starDirectorWriterCreator(details, null)
    const director = starDirectorWriterCreator(credits, 'Director')
    const writer = starDirectorWriterCreator(credits, 'Writer')

    useEffect(() => {
      if ('backdrop_path' in details) {
        setStarDirectorWriterResults({
          creator: creator?.[1] || null,
          director: director?.[1],
          writer: writer?.[1]
        })
      }
        // setStarDirectorWriterResults(creator[1] || director[1] || writer[1])
  ...
  return (
    <>
                ...
                <ul className="grid grid-flow-col overflow-x-auto justify-start gap-6 py-6">
                  {
                    creator[1]?.map(
                      (creator, i) => (
                        <li key={i} >
                          <CreateStarDirectorWriterCreator person={creator} />
                        </li>
                    ))
                  }
                </ul>
          ...
          {
          selected === 'director' &&  credits.crew && 
            director[0] &&
              <div className="p-4 my-10 flex justify-center flex-wrap">
                <p className="text-lg w-full text-center">
                  {
                ...
                <ul className="grid grid-flow-col overflow-x-auto justify-start gap-6 py-6">
                  {
                    director[1]?.map(
                      (director, i) => (
                      <li key={i} >
                        <CreateStarDirectorWriterCreator person={director} />
                      </li>
                    ))
                  }
                </ul>
                ...
                >
                  {
                    writer[1]?.map(
                      (writer, i) => {
                        return  (
                          <li key={i} >
                            <CreateStarDirectorWriterCreator person={writer} />
                          </li>
                        )
                      })
                  }
...
function CreateStarDirectorWriterCreator({person}: {person: Star}) {
    ...
    (async () => {
      try {
        ...
        const personDetails: MediaTypeInfoType['personDetails'] = await axios(
          `https://api.themoviedb.org/3/person/${person.id}?language=en-US`,
          TMDBOptions
        ).then(res => res.data);

        const combinedCredits: CombinedCredits = await axios(
          `https://api.themoviedb.org/3/person/${person.id}/combined_credits?language=en-US`,
          TMDBOptions
        ).then(res => res.data);

        // console.log(personDetails, combinedCredits)

        const bestCombineCredits = getBestCombinedCredit(combinedCredits.cast)
        // console.log(bestCombineCredits, person.id)
        ...
        const tvCast: TVcast = {
          ...
          name: person.name,
          original_name: '',
          popularity: bestCombineCredits.popularity,
          profile_path: person.profile_path || bestCombineCredits.poster_path
        };
  ...
  const bg = getTailwindColorUsingNumber(person.id, false, smallAppleDevice).split(' ')
      ...
      >
          ...
          <div className="w-full h-full">
            <Image
              src={person.profile_path
                ? ImagePath + person.profile_path : '/no-image-2.webp'}
              alt="crew image"
              width={100}
              height={100}
              placeholder="blur"
        ...
        <foreignObject x="82" y="38" width="79" height="50">
          <div className="w-full h-full px-2 py-1">
            <p className=" font-semibold flex justify-end [font-size:12px] leading-[14px]">
              <span>
        ...
        <foreignObject x="2" y="89" width="162" height="151">
          <div className="w-full h-full px-2 [font-size:12px] leading-[14px] grid ">
            <div className="bg-red-500p">
              <h3 className="font-bold text-[14px]">{person.name}</h3>
              <p className="py-1">
                {personTVCast?.character 
                  ? personTVCast.character === 'Self' ? person.name 
                  : personTVCast.character : 'N/A'
                }
              </p>
              <p className="py-1">{personDetail.known_for_department || 'Writing'}</p>
            </div>
            ...
            <div className=" grid items-end">
              ...
              <p>{getGenderByNumber(person.gender)}</p>
            </div>
          </div>
        </foreignObject>
  ...
  (
      ...
      >
        ...
        <foreignObject x="2" y="89" width="162" height="151">
          <div className="w-full h-full px-2 [font-size:12px] leading-[14px] grid ">
            <div className="bg-red-500p">
              <h3 className="font-bold text-sm text-[14px]">{person.name}</h3>
              <p className="py-1">
                {personTVCast?.character 
                  ? personTVCast.character === 'Self' ? person.name 
                  : personTVCast.character : 'N/A'
...
export type ButtonOption = 'star' | 'director' | 'writer' | 'creator'

// (parameter) personData: {
//   id: number;
//   name: string;
